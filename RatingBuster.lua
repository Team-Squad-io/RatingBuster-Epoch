...
-- StatLogic is loaded from RatingBusterStatLogic.lua
-- Note: StatLogic functions are now in a separate file for better organization

-- Store the global GetItemInfo function
local _GetItemInfo = GetItemInfo
local GetItemInfo = _GetItemInfo
...
--]]
	...
	showMP5FromSpi = true, -- Classic: Spirit is important for mana regen
	showMP5NCFromSpi = true, -- Classic: Spirit mana regen when not casting
	showHP5FromSpi = true, -- Classic: Spirit also gives health regen
	showSpellDmgFromSpi = false, -- Priest (disabled for Classic)
	showHealingFromSpi = false, -- Priest (disabled for Classic)
	showSpellCritFromSpi = false, -- Mage (disabled for Classic)
...
function RatingBuster:OnInitialize()
	print("|cff00ff00RatingBuster:|r Initialized with embedded StatLogic functionality")
...
function RatingBuster.ProcessTooltip(tooltip, name, link, ...)
	...
	if profileDB.enableAvoidanceDiminishingReturns then
		...
		-- Get equipped item stats using embedded StatLogic
		if StatLogic and StatLogic.GetDiffID then
			...
			-- StatLogic not available - use fallback
			print("RatingBuster: StatLogic not available at line 2639")
			equippedSum = {}
		end
